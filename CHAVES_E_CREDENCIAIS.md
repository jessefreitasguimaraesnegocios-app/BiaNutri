...
**Admin (acesso total sem pagamento e sem trial):** configurado no **Supabase**, tabela `public.admin_emails`. Adicione uma linha por e-mail (coluna `email`). Quem logar com esse e-mail terá acesso total. Veja migration `004_admin_emails.sql` e Table Editor no Dashboard.
